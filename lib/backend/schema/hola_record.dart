...
class HolaRecord extends FirestoreRecord {
  HolaRecord._(
    super.reference,
    super.data,
  ) {
  ...
  static CollectionReference get collection =>
      FirebaseFirestore.instance.collection('hola');

  static Stream<HolaRecord> getDocument(DocumentReference ref) =>
      ref.snapshots().map((s) => HolaRecord.fromSnapshot(s));

  static Future<HolaRecord> getDocumentOnce(DocumentReference ref) =>
      ref.get().then((s) => HolaRecord.fromSnapshot(s));

  static HolaRecord fromSnapshot(DocumentSnapshot snapshot) => HolaRecord._(
        snapshot.reference,
        mapFromFirestore(snapshot.data() as Map<String, dynamic>),
      );

  static HolaRecord getDocumentFromData(
    Map<String, dynamic> data,
    DocumentReference reference,
  ) =>
      HolaRecord._(reference, mapFromFirestore(data));

  @override
  String toString() =>
      'HolaRecord(reference: ${reference.path}, data: $snapshotData)';
...
Map<String, dynamic> createHolaRecordData({
...
class HolaRecordDocumentEquality implements Equality<HolaRecord> {
  const HolaRecordDocumentEquality();

  @override
  bool equals(HolaRecord? e1, HolaRecord? e2) {
  ...
  @override
  int hash(HolaRecord? e) => const ListEquality().hash([
        e?.owner,
        e?.usersAssigned,
        e?.projectName,
        e?.description,
  ...
  @override
  bool isValidKey(Object? o) => o is HolaRecord;
}
